...
function buildFfmpegArgs(
  ...
  // Stream copy only if same format AND no sample rate/bit depth change requested
  const canStreamCopy =
  ...
  if (canStreamCopy) {
    ...
    switch (targetFormat) {
      case "wav": {
        ...
        const sampleFmt = fmtMap[bitDepth] || "pcm_s16le";
        args.push("-codec:a", sampleFmt, "-map_metadata", "0");
        break;
      }
      case "flac":
        args.push("-codec:a", "flac", "-map_metadata", "0");
        // flac encoder defaults to 16-bit; 24-bit needs explicit flags
        if (bitDepth === "24") {
          args.push("-sample_fmt", "s32", "-bits_per_raw_sample", "24");
        }
        // 32-bit is downgraded to 24-bit in field validation above
        break;
      case "mp3":
      default:
        args.push(
  ...
  // Sample rate (-ar) — applies to all formats
...
router.post("/convert", (req, res) => {
  ...
  upload(req, res, async (err) => {
      ...
      if (err instanceof multer.MulterError) {
        ...
        if (err.code === "LIMIT_FILE_COUNT") {
          return res.status(400).json({
            error: `单次最多上传 ${req.app.locals.CONFIG.MAX_FILES_PER_REQUEST} 个文件`,
          });
        }
    ...
    // Read sample rate and bit depth
    let sampleRate = req.body.sampleRate || "original";
    let bitDepth = req.body.bitDepth || "original";

    // Validate sample rate values
    if (!["original", "44100", "48000", "96000"].includes(sampleRate)) {
      cleanupFiles(req.files);
      return res.status(400).json({ error: "不支持的采样率" });
    }
    // Validate bit depth values
    if (!["original", "16", "24", "32"].includes(bitDepth)) {
      cleanupFiles(req.files);
      return res.status(400).json({ error: "不支持的位深度" });
    }

    // Safety net: MP3 (libmp3lame) does not support 96000 Hz
    if (targetFormat === "mp3" && sampleRate === "96000") {
      sampleRate = "48000";
    }
    // Safety net: FLAC max is 24-bit; downgrade 32-bit to 24-bit
    if (targetFormat === "flac" && bitDepth === "32") {
      bitDepth = "24";
    }
    ...
    try {
      ...
      for (let i = 0; i < validationResults.length; i++) {
        ...
        if (
          r.status === "rejected" ||
          !r.value ||
          !ACCEPTED_FORMATS.includes(r.value.ext)
        ) {
      ...
      if (
        queue.size + queue.pending + req.files.length >
        req.app.locals.CONFIG.MAX_QUEUE_SIZE
      ) {
        cleanupFiles(req.files);
            ...
            proc.on("close", (code) => {
              ...
              if (code === 0) {
                ...
                if (t) {
                  ...
                  t.percent = 100;
                  t.downloadUrl =
                    "/downloads/" + encodeURIComponent(outputFilename);
                }
      ...
      return res
        .status(400)
        .json({ error: validationErr.message || "文件校验失败" });
    }
  });
});
