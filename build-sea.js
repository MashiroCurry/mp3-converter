...
console.log(
  `   ffmpeg.exe embedded (${(fs.statSync(ffmpegSrc).size / 1024 / 1024).toFixed(1)} MB → ${(Buffer.byteLength(ffmpegB64, "utf8") / 1024 / 1024).toFixed(1)} MB base64)`,
);
...
mkdir(distRoutes);
fs.copyFileSync(
  path.join(ROOT, "routes", "convert.js"),
  path.join(distRoutes, "convert.js"),
);
fs.copyFileSync(
  path.join(ROOT, "routes", "progress.js"),
  path.join(distRoutes, "progress.js"),
);
...
const bundleSize = (
  fs.statSync(path.join(DIST, "sea-bundle.js")).size /
  1024 /
  1024
).toFixed(1);
...
console.log(
  `   sea-prep.blob generated (${(fs.statSync(path.join(DIST, "sea-prep.blob")).size / 1024 / 1024).toFixed(1)} MB)`,
);
