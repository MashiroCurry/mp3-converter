...
- **高级音频选项** — 支持选择采样率（原始 / 44100 / 48000 / 96000 Hz）和位深度（原始 / 16 / 24 / 32 float），与目标格式一同配置
...
- **字段名**: `sampleRate` — `original` / `44100` / `48000` / `96000`（可选，默认 `original`；仅重编码时生效，流拷贝时忽略）
- **字段名**: `bitDepth` — `original` / `16` / `24` / `32`（可选，默认 `original`；仅 WAV/FLAC 输出时生效，MP3 忽略）
