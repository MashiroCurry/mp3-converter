...
function securityHeaders(req, res, next) {
  ...
  res.setHeader(
    ...
    "default-src 'self'; script-src 'self' https://www.googletagmanager.com https://www.google-analytics.com https://analytics.google.com https://*.baidu.com https://plausible.io; img-src 'self' data: https://www.google-analytics.com https://www.googletagmanager.com https://*.baidu.com; connect-src 'self' https://www.google-analytics.com https://*.baidu.com https://plausible.io; style-src 'self' 'unsafe-inline'; frame-ancestors 'none'; form-action 'self'",
  );
  next();
}
...
if (fs.existsSync(ffmpegBinPath)) {
  ...
  if (stat.size < 1024 * 1024) {
    console.error(
      `WARNING: ffmpeg binary at ${ffmpegBinPath} appears invalid (${(stat.size / 1024).toFixed(0)} KB)`,
    );
  } else {
    console.log(
      `ffmpeg: ${(stat.size / 1024 / 1024).toFixed(1)} MB at ${ffmpegBinPath}`,
    );
  }
}
...
app.get("/downloads/:filename", (req, res) => {
  ...
  const downloadName = filename.replace(
    /_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(\.(mp3|wav|flac))$/i,
    "$1",
  );
...
function cleanupFiles() {
  ...
  for (const dir of dirs) {
    fs.readdir(dir, (err, files) => {
      if (err) {
        if (err.code !== "ENOENT")
          console.error(`cleanup readdir ${dir}:`, err.message);
        return;
      }
      for (const file of files) {
        ...
        fs.stat(filepath, (err, stats) => {
          if (err) {
            if (err.code !== "ENOENT")
              console.error(`cleanup stat ${filepath}:`, err.message);
            return;
          }
          if (now - stats.mtimeMs > CONFIG.FILE_MAX_AGE_MS) {
            fs.unlink(filepath, (err) => {
              if (err && err.code !== "ENOENT")
                console.error(`cleanup unlink ${filepath}:`, err.message);
            });
          }
        });
      }
...
function cleanupTaskMap() {
  ...
  for (const [id, task] of tasks) {
    if (
      ...
      now - task.createdAt > CONFIG.TASK_MAP_MAX_AGE_MS
    ) {
      stale.push(id);
    }
  }
