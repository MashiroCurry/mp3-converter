...
function renderFileList() {
  ...
  selectedFiles.forEach((file, i) => {
    ...
    row.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
...
function toggleFormatOptions() {
  ...
  const format = checked ? checked.value : "mp3";
  const isMp3 = format === "mp3";
  const isFlac = format === "flac";

  // Bitrate visibility
  bitrateSelector.style.display = isMp3 ? "block" : "none";

  // Bit depth visibility (MP3: not applicable)
  const bitDepthRow = document.getElementById("bitDepthRow");
  if (bitDepthRow) bitDepthRow.style.display = isMp3 ? "none" : "";

  // Constraint 1: MP3 (libmp3lame) does NOT support 96000 Hz
  const srOpt96000 = document.querySelector(
    '#sampleRate option[value="96000"]',
  );
  if (srOpt96000) {
    if (isMp3) {
      srOpt96000.disabled = true;
      const srEl = document.getElementById("sampleRate");
      if (srEl && srEl.value === "96000") srEl.value = "48000";
    } else {
      srOpt96000.disabled = false;
    }
  }

  // Constraint 2: FLAC official max is 24-bit
  const bdOpt32 = document.querySelector('#bitDepth option[value="32"]');
  if (bdOpt32) {
    if (isFlac) {
      bdOpt32.disabled = true;
      bdOpt32.textContent = "32 float (FLAC 不支持)";
      const bdEl = document.getElementById("bitDepth");
      if (bdEl && bdEl.value === "32") bdEl.value = "24";
    } else {
      bdOpt32.disabled = false;
      bdOpt32.textContent = "32 float";
    }
  }
}
...
convertBtn.addEventListener("click", () => {
  ...
  const targetFormatEl = document.querySelector(
  ...
  formData.append("sampleRate", sampleRateEl ? sampleRateEl.value : "original");

  const bitDepthEl = document.getElementById("bitDepth");
  formData.append("bitDepth", bitDepthEl ? bitDepthEl.value : "original");
...
function listenBatchProgress(taskIds, targetFormat, initialQueueSize) {
  ...
  for (let i = 0; i < taskIds.length; i++) {
    ...
    item.innerHTML = `<span class="fpi-name">${escapeHtml(selectedFiles[i].name)}</span>
    ...
    if (queueSize > 0) {
      const pos =
        queueSize >= taskIds.length
          ? queueSize - taskIds.length + i + 1
          : i + 1;
  ...
  const es = new EventSource(
    "/api/batch-progress?ids=" + encodeURIComponent(taskIds.join(",")),
  );
  ...
  es.addEventListener("complete", (e) => {
    ...
    resultItem.innerHTML = `<span>${escapeHtml(item ? item.nameEl.textContent : data.taskId)}</span>
  ...
  es.addEventListener("task-error", (e) => {
    ...
    if (taskId) {
      ...
      resultItem.innerHTML = `<span>${escapeHtml(item ? item.nameEl.textContent : taskId)}</span>
...
// Returning donor
if (
  localStorage.getItem("donated") &&
  !sessionStorage.getItem("thanked_shown")
) {
...
function showDonateCta(hasAnySuccess) {
  ...
  donateCta.innerHTML =
    '<p>如果帮到了你，可以请开发者喝杯咖啡 ☕</p><button class="btn-donate-inline" id="donateCtaBtn">支持开发</button>';
